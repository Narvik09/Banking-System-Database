...
-- example
call create_account('Alex Hamilton', 'Victoria Street, London', '2000-08-13', '{"6361876389"}', 'checkings', 4, 2);

-- procedure to create A LOAN ACCOUNT )))))(*$%^&*()(*&^%$#@@#$%^&*()_(*&^%$#@#$%^&*&^%$#)))
...
BEGIN
    ...
    SELECT * INTO temp_pass FROM CAST(md5(random()::TEXT) AS VARCHAR(50));
    SELECT * INTO temp_len FROM array_length(c_phone, 1);
    IF ((temp_len = 1 OR temp_len = 2) AND loan_amt >=1000.0 AND loan_amt <= 10000000.0 ) THEN
    BEGIN
        -- this part is for account and customer table
        INSERT INTO access (username, password) VALUES (temp_uname, temp_pass) RETURNING login_id INTO temp_lid;
        INSERT INTO customer (cust_name, cust_address, cust_DOB, emp_id, login_id) VALUES (c_name, c_add, c_DOB, e_id, temp_lid) RETURNING cust_id INTO temp_cid;
        INSERT INTO customer_phoneno (cust_id, cust_phoneno) SELECT temp_cid, UNNEST(c_phone);
        INSERT INTO account (account_type, balance, branch_id) VALUES (a_type, -(l_amt + l_amt* l_int), b_id) RETURNING account_no INTO temp_ano;
        INSERT INTO access_account VALUES (temp_lid, temp_ano);
        INSERT INTO customer_account VALUES (temp_cid, temp_ano);
        -- loan table shit
...
-- procedure to PAY LOANSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSS
-- use account_no, loan_id and amount
...
BEGIN
    IF EXISTS (SELECT cust_id FROM customer_loan WHERE loan_id = lo_id AND cust_id =  (SELECT cust_id FROM customer_account where account_no = acc_no)) THEN
        ...
        SELECT loan_interest INTO temp_interest FROM loan WHERE loan_id = lo_id;
        -- SELECT loan_amt INTO tempFROM loan WHERE loan_id = lo_id
        IF (temp_l_status ==1) THEN
            IF (amt>=1000.0 AND amt <= 10000000.0) THEN
            BEGIN
                ...
                INSERT INTO transactions (trans_type, trans_amt, trans_date, account_no, cust_id, emp_id, login_id) VALUES
                ('loan payment', amt, now()::timestamp, id, temp_cid, temp_eid, temp_lid);
                INSERT INTO payment(pay_amount, pay_date, loan_id, loan_interest) VALUES (amt, now()::timestamp, lo_id, temp_interest);
                RAISE NOTICE 'Successfull Loan Payment! Balance amount : %', (temp_amt + amt);
...
CREATE OR REPLACE FUNCTION show_transaction_log(id INT, startDate DATE, endDate DATE)
RETURNS TABLE (
    transaction_id INT,
...
BEGIN
    ...
    IF ((temp_len = 1 OR temp_len = 2) AND loan_amt >=1000.0 AND loan_amt <= 10000000.0 ) THEN
       BEGIN
        ...
        INSERT INTO customer (cust_name, cust_address, cust_DOB, emp_id, login_id) VALUES (c_name, c_add, c_DOB, e_id, temp_lid) RETURNING cust_id INTO temp_cid;
        INSERT INTO customer_phoneno (cust_id, cust_phoneno) SELECT temp_cid, UNNEST(c_phone);
        INSERT INTO account (account_type, balance, branch_id) VALUES (a_type, -l_amt, b_id) RETURNING account_no INTO temp_ano;
        INSERT INTO access_account VALUES (temp_lid, temp_ano);
        INSERT INTO customer_account VALUES (temp_cid, temp_ano);
        -- loan table shit
        INSERT INTO loan (loan_type, loan_status, amount, interest_rate, branch_id, emp_id) VALUES (l_type, 1, l_amt, l_int, b_id, e_id) RETURNING loan_id INTO temp_loanid;
...
-- Triggersssssssssssssssssssssssssssssssssssssssssssssssssssss

-- Triggers for pay loan procedure
...
begin
    ...
    if(interest_percent > new.loan_int or interest_percent < new.loan_int) THEN
...
GRANT EXECUTE ON PROCEDURE update_customer, deposit_amount, withdraw_amount, transfer_amount, update_creds TO employee;
-- for Branch admin
...
-- for customer views
-- when large number of customers use the same function over and over again,
-- it might lead to increased number of views. Have to think of a way to delete them
...
-- older code (may have some correct changes in roles)
-- for employees

GRANT all on customer, customer_account, customer_phoneno, account, transactions, access_account, loan, customer_loan, branch_loan to employee;
...
transfer_amount, update_creds, pay_loan, create_loan_account TO employee;

-- for Branch admin
-- why cant this guy see transactions
GRANT all on branch, employee, customer, account, access_account, customer_account, loan, customer_loan, branch_loan to branch_admin; 

GRANT execute ON FUNCTION show_transaction_log, view_balance, employee_assists TO branch_admin;

GRANT execute ON PROCEDURE update_customer, update_employee, deposit_amount, withdraw_amount,
transfer_amount, update_creds, pay_loan, create_loan_account TO branch_admin;
